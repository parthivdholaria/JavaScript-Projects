...
class Player {
  ...
  strike (player, enemy, attackDmg) {
    ...
    let dmg = 1+Math.floor(Math.random()*attackDmg);
  ...
  heal (player) {
    ...
    const hpAmount = 1+Math.floor(Math.random()*5);
    ...
    return `${player.name} heals ${hpAmount}`;
...
class Game {
  ...
  play(p1, p2) {
    ...
    while (!this.isOver) {
      ...
      p1.strike(p1,p2,this.isOver)
      p1.heal(p1);
      p2.strike(p2,p1,this.isOver);
      p2.heal(p2);
...
let player1 = new Player("parthiv",100,10);
...
document.addEventListener('keydown', function(e) {
  ...
  if (e.key=='q' && gameState.isOver==false && p2.health>0) {
